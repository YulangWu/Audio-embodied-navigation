...
def gen_frqwavefield_fixed_robot_pos(base_model, wavelet, src_pos, wall_lines_list, model_cpml_size, nt, nx, nz, dt, nf,robot_x,robot_z):
    res = born_modeling.forward_model_with_2D_source(wavelet, src_pos, nx, nz, base_model,wall_lines_list, 
                        model_cpml_size, 
                        return_parameters=['illumination','wavefield'])
    ...
    real_frqwavefield = np.zeros((nf,(a.one_side*2+1),(a.one_side*2+1)))
    imag_frqwavefield = np.zeros((nf,(a.one_side*2+1),(a.one_side*2+1)))
    for i in range(a.one_side*2+1):
        for j in range(a.one_side*2+1):
            temp = np.fft.fft(forward_wavefield[:,robot_x-2+i,robot_z-2+j])

            real_frqwavefield[:nf,i,j] = np.real(temp[:nf].copy())
            imag_frqwavefield[:nf,i,j] = np.imag(temp[:nf].copy())
            del temp
            # plt.plot(forward_wavefield[:,robot_x-2+i,robot_z-2+j])
            # plt.show()
            # plt.plot(real_frqwavefield[:,i,j])
            # plt.show()
            # plt.plot(imag_frqwavefield[:,i,j])
            # plt.show()

    del forward_wavefield
...
# -----------------------------------------------------------------------------
    ...
    parser.add_argument("--num_shots", type=float, default=200, help="Number of shots in the floor plan")
    ...
    parser.add_argument("--f_peak", type=int, default=0.5, help="Peak frequency")
    parser.add_argument("--nf", type=int, default=256, help="spatial order.")
    ...
    frq_list = [i for i in range(1,200)] #frequency band from low frequency to high frequency
    ...
    for num_shot in range(len(source_pos)):
        ...
        res = gen_frqwavefield_fixed_robot_pos(base_model, wavelet, source_pos[num_shot], wall_lines_list, 
        model_cpml_size, nsteps, nx, nz, dt, a.nf,a.robot_x,a.robot_z)
        ...
        del real_frqwavefield
        del imag_frqwavefield
