...
parser.add_argument("--nf", type=int, default=10, help="useless")
    ...
    if 'real' in return_parameters:
        ...
        for file in file_list:
            if "real_frqwavefield" in file:
                ...
                wavefield_crop = wavefield[0
                    :a.nf,a.robot_x-a.one_side:a.robot_x+a.one_side+1,
                    a.robot_z-a.one_side:a.robot_z+a.one_side+1].copy()
                # wavefield.close()
                del wavefield
                # for i in range(299):
                #     plt.imshow(wavefield[i,:,:])
                #     plt.title('Real')
                #     plt.show()
                real_frq_wavefield.append(wavefield_crop)
...
def main():
    ...
    if a.mode == 'train' or a.mode == 'export':
        data_set = load_frq_wavefield_fix_window('train_dataset',return_parameters=['real','pos'])
    elif a.mode == 'test':
        a.n_epochs = 1 #no need to repeated training
        data_set = load_frq_wavefield_fix_window('test_dataset',return_parameters=['real','pos'])
    ...
    for epoch in range(a.n_epochs):  # iterative training
        ...
        for src_index in src_list: #loop over the different wavefield images
            ...
            input_data = np.zeros((1,a.nf,a.one_side*2+1,a.one_side*2+1)) #5-by-5
